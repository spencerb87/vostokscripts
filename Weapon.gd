...
func _input(event):
    ...
    if Input.is_action_just_pressed("secondary_optic"):
        if gameData.isAiming && activeOptic != null && activeOptic.attachmentData.secondary && activeOptic.secondary != null:
            ...
            if opticToggle:
                aimOffset = abs(raycast.position.y - (activeOptic.position.y + activeOptic.secondary.position.y))
            else:
...
func ReloadInput():
    ...
    if Input.is_action_just_pressed(("reload")) && (weaponData.action == 2 || weaponData.action == 3) && ( !gameData.isPreparing && !gameData.isInserting):
    ...
    if Input.is_action_just_pressed(("prepare")) && (weaponData.action == 2 || weaponData.action == 3):
    ...
    if Input.is_action_just_pressed(("insert")) && (weaponData.action == 2 || weaponData.action == 3):
